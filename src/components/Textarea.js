...
export default function Textarea(props) {
 ...
 <>
  <div className='container'style={{color:props.mode === "light"?"black":"white"}}>
    ...
    <button className="btn btn-primary mx-3  my-1" onClick={handleonclickup} >ChanegeToUpcase</button>
    <button className="btn btn-primary mx-3  my-1" onClick={handleonclicklo} >ChangeToLocase</button>
    <button className="btn btn-primary mx-3  my-1" onClick={Copy} >Copy</button>
    <button className="btn btn-primary mx-3  my-1" onClick={Clear} >Clear</button>
    <button className="btn btn-primary mx-3  my-1" onClick={space} >Remove Extra Space</button>
    
  </div>
  <div className='container  my -3'style={{color:props.mode === "light"?"black":"white"}}>
    <h3>Your Text Summery</h3>
    <p>{title.split(" ").filter((element)=>{return element.length!==0}).length} Words and {title.length} Alphabets</p>
    <p>{0.008 * title.split(" ").filter((element)=>{return element.length!==0}).length} Minutes Required To Read Text</p>
  </div>
  </>
